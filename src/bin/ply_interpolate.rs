...
fn run() -> Result<()> {
    ...
    .arg(Arg::new("density_radius")
     ...
     .arg(Arg::new("dist_func")
              .long("dist_func")
              .takes_value(true)
              .multiple_occurrences(false)
              .help("Define which distance fucnction to use"))
     .arg(Arg::new("mark_enlarged")
              .long("mark_enlarged")
              .takes_value(false)
              .multiple_occurrences(false)
    ...
    if method == "closest_with_ratio_average_points_recovery" {
        if two_way_interpolation {
            ...
            let (mut interpolated_pc, _prev_pc, _next_pc) =
                closest_with_ratio_average_points_recovery(prev_pc, next_pc, params); //sum of first 3 must equal 1
