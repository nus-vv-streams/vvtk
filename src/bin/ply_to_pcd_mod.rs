...
pub fn ply_to_pcd(output_dir: String, storage_type: PCDDataType, files: Vec<OsString>) {
    // let args: Args = Args::parse();

    let files_to_convert = filter_for_ply_files(files);
    ...
    'outer: for file_path in tqdm!(files_to_convert.into_iter()) {
        ...
        let header = {
            match vertex_parser.read_header(&mut f) {
                Ok(h) => h,
                Err(e) => {
                    println!("Failed to convert(Can not parser header) {:?}\n{e}", file_path.into_os_string());
                    continue;
                }
            }
        };
...
pub fn filter_for_ply_files(os_strings: Vec<OsString>) -> Vec<PathBuf> {
...
fn create_pcd(vertices: Vec<Vertex>) -> PointCloudData {
    let header = PCDHeader::new(
        ...
        vertices.len() as u64,
        PCDDataType::Ascii, // this is a placeholder value, it will be overwritten in write_pcd_file()
    )
    .unwrap();
