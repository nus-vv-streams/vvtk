...
impl Upsampler {
    ...
    fn euclidean_distance_3d(&self, point1: &PointXyzRgba, point2: &PointXyzRgba) -> f32 {
    ...
    fn get_middlepoint(&self, point1: &PointXyzRgba, point2: &PointXyzRgba) -> PointXyzRgba {
    ...
    fn get_circumference_order(&self, neighbours: &Vec<usize>, points: &Vec<PointXyzRgba>) -> Vec<usize> {
    ...
    pub fn upsample_grid(&self, vertices: Vec<PointXyzRgba>) -> Vec<PointXyzRgba> {
        let start = Instant::now();
        ...
        for source in 0..vertices.len() {
            ...
            match kd_tree.nearest(&[x, y, z], 9, &squared_euclidean){
                Ok(nearest) => {
                    let neighbours = nearest.iter().map(|(_, second)| **second).skip(1).collect();
