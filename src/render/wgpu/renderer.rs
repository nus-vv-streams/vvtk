...
pub fn parse_bg_color(bg_color_str: &str) -> Rgb {
    if bg_color_str.starts_with("rgb") {
        ...
        if !pattern.is_match(bg_color_str.as_bytes()) {
            panic!("Invalid background rgb color format, expected rgb(r,g,b) such as rgb(122,31,212)");
        }
        
        let rgb = bg_color_str[4..bg_color_str.len() - 1].split(',')
            .map(|s| s.parse::<u8>().unwrap())
            .collect::<Vec<_>>();
        return Rgb::new(rgb[0] as f64, rgb[1] as f64, rgb[2] as f64);
    }
    else if bg_color_str.starts_with("#") {
        let hex_num = u32::from_str_radix(&bg_color_str[1..], 16);
        if bg_color_str.len() != 7 && hex_num.is_err() {
            panic!("Invalid background hex color format, expected #rrggbb such as #7a1fd4");
        }
        return Rgb::from_hex(hex_num.unwrap())
    }
    else {
        panic!("Invalid background color format, expected rgb(r,g,b) or #rrggbb such as rgb(122,31,212) or #7a1fd4");
    }
}
    ...
    U: Renderable,
{
    pub fn new(
        ...
        Self {
            ...
            bg_color: parse_bg_color(bg_color_str),
        }
    }
}
