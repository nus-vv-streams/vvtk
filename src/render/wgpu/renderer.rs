...
pub fn parse_bg_color(bg_color_str: &str) -> Result<Rgb, &str> {
    if bg_color_str.starts_with("rgb") {
        ...
        if !pattern.is_match(bg_color_str.as_bytes()) {
            return Err("Invalid background rgb color format, expected rgb(r,g,b) such as rgb(122,31,212)");
        }
...
mod test {
    ...
    fn test_parse_bg_color() {
        assert_eq!(parse_bg_color("rgb(255,122,11)").unwrap(), Rgb::new(255f64, 122f64, 11f64));
        assert_eq!(parse_bg_color("#9ef244").unwrap(), Rgb::new(158f64, 242f64, 68f64));
        assert_eq!(parse_bg_color("#9EF24A").unwrap(), Rgb::new(158f64, 242f64, 74f64)); 
